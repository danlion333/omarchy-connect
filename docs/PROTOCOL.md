...
turns one loud syllable into a click louder than the syllable. The gain is on
the samples and not on the device: it goes on before the interpolation that
feeds the pipe, so nothing about how the source is declared moves with it.
