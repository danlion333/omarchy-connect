...
# it. Writes $STATE/<n>/verify.json and prints a verdict line last:
...
if ! node "$wt/tools/run-tests.mjs" $pkgs --report "$out/tests.json" 2>&1 | tail -25; then
...
if grep -q app <<<"$areas"; then
  hdr "typecheck"
  (cd "$wt/app" && npx tsc --noEmit 2>&1 | tail -15) || fail+=("typecheck")
fi
...
if ! phone_present; then
  ...
  [ ${#fail[@]} = 0 ] && echo "VERDICT phone-missing" || echo "VERDICT red ${fail[*]}"
  exit 0
fi
...
if [ "$class" = apk ] || grep -q '^app' <<<"$(git -C "$wt" diff --name-only master...HEAD | grep -v '^app/test')"; then
  ...
  if [ -d "$wt/app/android" ]; then
    (cd "$wt/app/android" && ./gradlew -q assembleRelease 2>&1 | tail -20) \
...
[ ${#fail[@]} = 0 ] && echo "VERDICT green" || echo "VERDICT red ${fail[*]}"
